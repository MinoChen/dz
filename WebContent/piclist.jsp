			...
			<c:if test="${!empty pb.list}">
				<ul>
					<c:forEach items="${pb.list}" var="pic">
						<li>
						<span style="font-size: 16px;"><c:out value="${pic.pic_publishTime}" /></span> 
						<a href="${basePath}PicServlet?method=findPicByPic_id
						&pic_id=<c:out value="${pic.pic_id}"/>
						&category_id=<c:out value="${category.category_id}" />">
						<c:out value="${pic.pic_title}" />
						</a></li>
					</c:forEach>
				</ul>
